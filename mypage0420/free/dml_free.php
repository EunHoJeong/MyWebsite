<?php
    ...
    $nicname = isset($_SESSION['user_nicname']) ? $_SESSION['user_nicname'] : null;
    ...
    if (isset($_POST["mode"]) && $_POST["mode"] == "insert") {
        ...
        if (isset($_FILES["upfile"])) {
            ...
            $sql = "INSERT INTO `free` VALUES (null,'$q_userid','$nicname','$q_subject','$q_content','$regist_day',0,'$is_html','$upfile_name','$copied_file_name','$type[0]');";
        ...
        $userid = $_SESSION['user_id'];
        $q_userid = mysqli_real_escape_string($con, $userid);
        $sql = "select * from users where id = '$q_userid'";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Error: ' . mysqli_error($con));
        }
        ...
        if (!$rowcount) {
            ...
            $hit = input_set($_POST["hit"]);
            $q_nicname = isset($_SESSION['user_nicname']) ? mysqli_real_escape_string($con, $_SESSION['user_nicname']) : null;
            $q_content = mysqli_real_escape_string($con, $content);
            $q_parent = mysqli_real_escape_string($con, $parent);
            $regist_day = date("Y-m-d (H:i)");

            $sql = "INSERT INTO `free_ripple` VALUES (null,'$q_parent','$q_userid','$q_nicname','$q_content','$regist_day')";
            $result = mysqli_query($con, $sql);
            if (!$result) {
                die('Error: ' . mysqli_error($con));
            }
